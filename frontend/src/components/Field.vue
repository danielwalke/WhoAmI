<template>
    <div class="w-full h-full flex flex-col  overflow-y-auto">
          ...
          <div class="flex-1 w-full">
              ...
              <div
          class="grid grid-cols-2 lg:grid-cols-6 gap-x-8 gap-y-4 p-2  w-full h-full" :class="isExpanded ? '':''"
          >
        ...
        <div v-if="isDesktop && hasConnection" class="flex flex-col w-full overflow-hidden transition-all ease-in-out duration-300 bg-black/90 pb-4" :class="isExpanded ? 'absolute bottom-0 left-0 right-0 h-4/5': 'h-[250px]'">
            <div @click="expand" class="flex justify-center pt-2 h-12 w-full items-center cursor-pointer bg-black/90">
                <svg v-if="!isExpanded" class="hover:scale-110 hover:cursor-pointer animate-bounce" width="96" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <svg  class="hover:scale-110 hover:cursor-pointer animate-bounce" v-if="isExpanded" width="96" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
